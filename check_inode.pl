...
if ( $pid == 0 )
{
    exec("df -i | grep -v Filesystem > checkinode.txt");   #forks a chlid for df -h command
}
else
{
    eval {
    ...
    open my $file, "<checkinode.txt" or "die cannot open file"; 
    ...
    foreach $mount (@mount)  #processing line by line
    {
        ...
        if ( $new_mount eq $x ) #mount value is excluded and goes to the next line in the array 
        {
            next;
        }
...
{
    print <<ENDOF
USAGE ./disk_inode -c [value] -w [value] -x [value]
